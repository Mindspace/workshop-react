...
export type StreamSelector<T> = (state: any) => T;

const NOOP: StreamSelector<any> = (s: any) => s;

/**
 * Quick util to read 1st value emitted from BehaviorSubject/replay streams
 */
export function readFirst<T>(source: Observable<any>, selector?: StreamSelector<T>): T {
 ...
 */
export function readFirstAsync<T>(source: Observable<any>, selector?: StreamSelector<T>): Promise<T> {
